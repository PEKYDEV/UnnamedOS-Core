...
The workspace-owned `boot-protocol`, `kernel-image`, `memory-layout`, and `xtask` crates have no external runtime dependencies. The page-table planner, owner, materializer, verifier, and read-only CPUID/control-state probe are allocator-free project code and add no dependency or license exception. The `kernel` depends only on `boot-protocol`; the bootloader's `memory-layout` dependency is workspace-owned. No QEMU or firmware binary is distributed by this repository.
