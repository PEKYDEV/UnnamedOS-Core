...
The workspace-owned `boot-protocol`, `kernel-image`, and `xtask` crates have no external runtime dependencies. The `kernel` depends only on `boot-protocol`. No QEMU or firmware binary is distributed by this repository.
